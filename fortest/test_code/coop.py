...
for jj in range(0, 6):
	for kk in range(1, 7):
		...
		max_dimension = slide.dimensions

		he_result_iter = [[], []]
		beilv = max_dimension[0] / mason['dimensions'][0]
		countbodyMax = 0
		meanmax = 0
		detectmax = 0
		avermax = 0
		numbermax = 0
		averageBlockMax = 0
		for y in range(0, max_dimension[1] - 1000, 1000):
			he_result_iter[0].append([])
			he_result_iter[1].append([])
			for x in range(0, max_dimension[0] - 1000, 1000):
				...
				if countbody > 1000 * 1000 * 0.5:
					masonimg = mason['masonimg'][int(y / beilv):int((y + 1000) / beilv),
					           int(x / beilv):int((x + 1000) / beilv)]
					...
					for i in range(1, number):
						...
						j[labels == i] = 255
						totalBlock.append(cv2.countNonZero(j))
					averageBlock = int(numpy.averate(totalBlock) / number)

					a = cv2.cvtColor(region, cv2.COLOR_BGR2GRAY)
					fd, hog_image = hog(a, orientations=8, pixels_per_cell=(200, 200), cells_per_block=(1, 1),
					                    block_norm='L2-Hys', visualise=True)
					for i in range(0, len(fd)):
						fd[i] = fd[i] * 100
						prediction = clf.predict([fd])

					he_result_iter[0][len(he_result_iter[0]) - 1].append([countbody, mean, detect[0], detect[1]])
					he_result_iter[1][len(he_result_iter[1]) - 1].append([number, averageBlock, prediction[0]])
					...
					he_result_iter[0][len(he_result_iter[0]) - 1].append([0, 0, 0, 0])
					he_result_iter[1][len(he_result_iter[1]) - 1].append([0, 0, 0])
				print (
					he_result_iter[0][len(he_result_iter[0]) - 1][
						len(he_result_iter[0][len(he_result_iter[0]) - 1]) - 1],
					he_result_iter[1][len(he_result_iter[1]) - 1][
						len(he_result_iter[1][len(he_result_iter[1]) - 1]) - 1])

		he_result_iter[0] = numpy.array(he_result_iter[0])
		he_result_iter[1] = numpy.array(he_result_iter[1])
		s, a, b, c = cv2.split(he_result_iter[0])
		d, e, f = cv2.split(he_result_iter[1])
		...
		he_result_iter[0] = cv2.merge([s, a, b, c])
		he_result_iter[1] = cv2.merge([d, e, f])
		cv2.imwrite(outputimage + '0.png', he_result_iter[0])
		cv2.imwrite(outputimage + '1.png', he_result_iter[1])

		outputFile = open(outputPath, 'a+')
		outputFile.write(
			name + ',' + str(countbodyMax) + ',' + str(meanmax) + ',' + str(detectmax) + ',' + str(avermax) + ',' + str(
				numbermax) + ',' + str(averageBlockMax) + '\n')
		outputFile.close()
